...
const { Host, Port, Username, Password} = require('../FTP_config/config.json')
const fs = require('fs');
                    ...
                    uuid(USER).then(player =>{
                        ...
                        fs.readFile('whitelist.json', (err, res) =>{
                            ...
                            if(localWL.filter((e) => {
                                ...
                                Interaction.reply({content: "This user is already whitelisted!"})
                                sftp.end();
                            }
                            else
                            {
                                ...
                                fs.writeFile('whitelist.json', JSON.stringify(localWL, null, 4), (err, res)=> {
                                    if(err){
                                        ...
                                        Interaction.reply({ content: 'Oops, could not write to file ;(('});
                                        sftp.end();
                                    } else {
                                        sftp.put(stream, 'whitelist.json').then(() => {
                                            sftp.end().then(() => {
                                                console.log(USER, ' added to whitelist')
                                                Interaction.reply({ content: `Added ${USER} to the whitelist! \n Enjoy playing!`});
                                            })
                                        })
                                    }
                                })
